          ...
          >
            ...
            <el-table-column prop="status" label="状态">
              <template #default="scope">
                <el-tag
                  v-if="scope.row.showStatusColumn"
                  :type="scope.row.status === 1 || scope.row.status === 0 ? 'success' : 'danger'"
                >
                  {{ scope.row.status === 1 || scope.row.status === 0 ? "正常" : "故障" }}
                </el-tag>
              </template>
            </el-table-column>
...
export default {
  ...
  created() {
    localStorage.setItem("host", "219.216.65.171")
    localStorage.setItem("port", "8086")
    this.fetchFirstData()
  },
  methods: {
    ...
    fetchSecondData() {
      ...
      const queryHost = localStorage.getItem("host")
      const queryPort = localStorage.getItem("port")
